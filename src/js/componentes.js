...
const divTarjetaList  = document.querySelector('.tarjeta-list');
const divMas          = document.querySelector('.mas');
const txtInput        = document.querySelector('.new-todo');
const btnBorrar       = document.querySelector('.clear-completed');
const ulFiltros       = document.querySelector('.filters');
const anchorFiltros   = document.querySelectorAll('.filtro'); 

export const crearTarjetaHtml = (tarjeta) => {
    const htmlTarjeta = `
        <li class="tar" data-id="${tarjeta.id}">
            <div class="view">
                <label>${ tarjeta.tarjeta }</label>
    ...
    divTarjetaList.append(div.firstElementChild);
...
//Eventos
divMas.addEventListener('click', (event) => {
        const nuevaTarjeta = new Tarjeta(divMas.value);
        tarjetaList.nuevaTarjeta(nuevaTarjeta);
        crearTarjetaHtml(nuevaTarjeta);
        divMas.value = '';
});
...
divTarjetaList.addEventListener('click', (event) => {
    const nombreElemento = event.target.localName; //Puedo identificar la parte del li a la que hice click
    //input, label o button

    const tarjetaElemento = event.target.parentElement.parentElement; //Obtengo el li con el id del elemento
    const tarjetaId = tarjetaElemento.getAttribute('data-id');
    
    if (nombreElemento.includes('button')){
        //Hay que borrar la tarjeta
        tarjetaList.eliminarTarjeta( tarjetaId );
        //La referencia html también hay que borrarla
        divTarjetaList.removeChild( tarjetaElemento );
    }
});
